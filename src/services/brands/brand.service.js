        ...
        ratingDistribution: {
          1: {
            $size: {
              $filter: {
                input: '$reviewStats',
                cond: { $eq: [{ $round: ['$$this.avgRating', 0] }, 1] },
              },
            },
          },
          2: {
            $size: {
              $filter: {
                input: '$reviewStats',
                cond: { $eq: [{ $round: ['$$this.avgRating', 0] }, 2] },
              },
            },
          },
          3: {
            $size: {
              $filter: {
                input: '$reviewStats',
                cond: { $eq: [{ $round: ['$$this.avgRating', 0] }, 3] },
              },
            },
          },
          4: {
            $size: {
              $filter: {
                input: '$reviewStats',
                cond: { $eq: [{ $round: ['$$this.avgRating', 0] }, 4] },
              },
            },
          },
          5: {
            $size: {
              $filter: {
                input: '$reviewStats',
                cond: { $eq: [{ $round: ['$$this.avgRating', 0] }, 5] },
              },
            },
          },
        },
...
const getBrandById = async (
  ...
  const stats = await Review.aggregate([
    ...
    },
    {
      $project: {
        ...
        ratingDistribution: {
          1: {
            $size: { $filter: { input: '$ratingCounts', cond: { $eq: ['$$this.rounded', 1] } } },
          },
          2: {
            $size: { $filter: { input: '$ratingCounts', cond: { $eq: ['$$this.rounded', 2] } } },
          },
          3: {
            $size: { $filter: { input: '$ratingCounts', cond: { $eq: ['$$this.rounded', 3] } } },
          },
          4: {
            $size: { $filter: { input: '$ratingCounts', cond: { $eq: ['$$this.rounded', 4] } } },
          },
          5: {
            $size: { $filter: { input: '$ratingCounts', cond: { $eq: ['$$this.rounded', 5] } } },
          },
        },
      },
    },
  ...
  const reviews = await Review.find(reviewQuery).sort(sort).skip(skip).limit(limit);
