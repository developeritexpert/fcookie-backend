...
const addReview = {
  [Segments.BODY]: Joi.object()
    .keys({
      ...
      reviewType: Joi.string().trim().valid('Product', 'Brand').insensitive().required().messages({
      ...
      productId: Joi.string().custom(objectIdValidation).allow('', null).optional(), // ✅ changed
      brandId: Joi.string()
        ...
        .when('reviewType', {
          ...
          then: Joi.required().messages({
            'any.required': 'Brand ID is required for brand reviews',
          }),
        })
        .when('reviewType', {
          is: 'Product',
          then: Joi.required().messages({
            'any.required': 'Brand ID is required when selecting a product',
          }),
        }),
      ...
      if (value.reviewType === 'Product') {
        ...
        if (hasProductId && hasShopifyId) {
          return helpers.message(
            'Provide only one: either productId or shopifyProductId (not both)'
          );
        }
